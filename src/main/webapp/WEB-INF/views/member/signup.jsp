...
<script>
	$(function() {
		...
		$("#id").blur(function(){
			var id = $("#id").val();
			$.ajax({
				url: 'idDupleCheck?id='+id,
				dataType:'json',
				success:function(resp){
					console.log(resp);
					if(resp==true){
						$("#idCheckDiv").text("사용중인 아이디입니다.");
						$("#idCheckDiv").css("color","red");
						$("#signUpBtn").attr("disabled",true);
					}else{
						if(idRegEx.test(id)){
							$("#idCheckDiv").text("사용가능한 아이디입니다.");
							$("#idCheckDiv").css("color","blue");
							$("#signUpBtn").attr("disabled",false);
						}else if(id==""){
							$("#idCheckDiv").text("아이디를 입력해주세요.");
							$("#idCheckDiv").css("color","red");
							$("#signUpBtn").attr("disabled",true);
						}else{
							$("#idCheckDiv").text("아이디는 대소문자 및 숫자 4~16자만 가능합니다. ");
							$("#idCheckDiv").css("color","red");
							$("#signUpBtn").attr("disabled",true);
						}
					}
	...
	<div class="container">
		<div class="row mt-5 pt-5">
			...
			<div class="col-8 signupBox  px-1 py-1">
				<div class="row">
					<div class="col-12 text-center h1">회원가입</div>
				</div>
